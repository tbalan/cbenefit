...
benefit_cv <- function(model_fit, treat_name, folds = 10, times = 10, reproducible = FALSE, match_by = "benefit", data = NULL, matchit_args = list()) {
  ...
  if(match_by == "benefit") {
    ...
    matchit_args$data <- data.frame(treat = treat, pred_ben = pred_ben)
    match <- do.call(function(...) matchit(...)$match.matrix, matchit_args)
  }
#   browser()

    # match <- MatchIt::matchit(treat ~ pred_ben, data = data.frame(x = TRUE))$match.matrix
  ...
  # observed benefit, obs_pred data, nb
  if(inherits(model_fit$y, "Surv")) {
  ...
  for(iter in 1:times) {
    ...
    # matchit needs a non-empty data frame, so I just give something there

    if(isTRUE(reproducible)) set.seed(iter)
    # match by predicted benefit
  ...
  if(isTRUE(reproducible)) set.seed(1)
    if(match_by == "benefit") {
      ...
      matchit_args$data <- data.frame(treat = treat, ave_pred_ben = ave_pred_ben)
      match <- do.call(function(...) matchit(...)$match.matrix, matchit_args)
    }
